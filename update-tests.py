...
SKIP_LINE = 'if (builtin.zig_backend == .stage2_spirv) return error.SkipZigTest;'
...
ap.add_argument('--timeout', type=int, default=30, help='Maximum number of seconds to wait for a test to finish')
ap.add_argument('--api', choices=['opencl','vulkan'], default='opencl', help='API to use')
ap.add_argument('--no-fp16', default=False, action='store_true', help='Drop float16 from the target features (some drivers, e.g. POCL, lack cl_khr_fp16)')
ap.add_argument('--jobs', '-j', type=int, default=None, help='Number of parallel workers (default: all cores). Lower this if tests time out from CPU contention, e.g. with slow CPU drivers like POCL')
...
class Test:
    def __init__(
        ...
        self.error = None
        # True only after a successful run, so unexecuted tests keep their guard.
        self.passed = False
    ...
    def update(self, tests):
        ...
        for line in self.lines:
            if line.startswith('test '):
                test = tests[i]
                i += 1
            elif SKIP_LINE in line and test.passed and not test.flaky:
                continue
...
def run_test(test, tmp_path, tmp_file):
    ...
    try:
        ...
        features = '+int64+float64+generic_pointer'
        if not args.no_fp16:
            features += '+float16'
...
total_passed = 0
try:
    with mp.Pool(processes=args.jobs) as p:
